...
def dyer_injector(cpres, inj_dia, lden, inj_pdrop, hl, manifold_P, vpres):
    ...
    if inj_pdrop < 3 * 10 ** 5:
        print(
            "accuracy warning: injector pdrop so low that 2-phase Dyer model no longer applies.",
    ...
    mdot_hem = (
        Cd * (((inj_dia / 2) ** 2) * np.pi) * rho2 * ((2 * -(hl - h2)) ** (1 / 2))
    )
    if vpres < cpres:
        raise RuntimeError(
            "injector pdrop lower than vapour pressure",
...
def Z2_solve(temp1, Z1, vmass1, vmass2, gamma_n2o, zdat, pdat):
    ...
    if np.sign(temp2_delta(min(zdat), Z1, vmass1, vmass2, gamma_n2o)) == np.sign(
        temp2_delta(max(zdat), Z1, vmass1, vmass2, gamma_n2o)
    ):
        return "numerical instability"
    else:
        Z2_solved = scipy.optimize.bisect(
            temp2_delta,
            min(zdat),
            max(zdat),
...
def ball_valve_K(Re, d1, d2, L):
    ...
    if Re < 2500:
        K = (
            (2.72 + ((d2 / d1) ** 2) * ((120 / Re) - 1))
            * (1 - ((d2 / d1) ** 2))
            * (((d1 / d2) ** 4) - 1)
        )
    else:
        K = (
            (2.72 + ((d2 / d1) ** 2) * (4000 / Re))
            * (1 - ((d2 / d1) ** 2))
            * (((d1 / d2) ** 4) - 1)
        )
...
def thermophys(temp):
    ...
    lden = 452 * (
        np.e
        ** (
            1.72328 * ((1 - (temp / 309.57)) ** (1 / 3))
            + -0.8395 * ((1 - (temp / 309.57)) ** (2 / 3))
            + 0.5106 * (1 - (temp / 309.57))
            + -0.10412 * ((1 - (temp / 309.57)) ** (4 / 3))
        )
    )
    ...
    hl = (
        -200
        + 116.043 * ((1 - (temp / 309.57)) ** (1 / 3))
        + -917.225 * ((1 - (temp / 309.57)) ** (2 / 3))
        + 794.779 * (1 - (temp / 309.57))
        + -589.587 * ((1 - (temp / 309.57)) ** (4 / 3))
    ) * 1000

    hg = (
        -200
        + 440.055 * ((1 - (temp / 309.57)) ** (1 / 3))
        + -459.701 * ((1 - (temp / 309.57)) ** (2 / 3))
        + 434.081 * (1 - (temp / 309.57))
        + -485.338 * ((1 - (temp / 309.57)) ** (4 / 3))
    ) * 1000
    ...
    c = (
        ...
        * (
            1
            + 0.023454 * ((1 - (temp / 309.57)) ** (-1))
            + -3.80136 * (1 - (temp / 309.57))
            + 13.0945 * ((1 - (temp / 309.57)) ** 2)
            + -14.5180 * ((1 - (temp / 309.57)) ** 3)
        )
    ) * 1000
    ...
    vpres = 7251000 * (
        ...
        ** (
            (1 / (temp / 309.57))
            * (
                -6.71893 * (1 - (temp / 309.57))
                + 1.35966 * ((1 - (temp / 309.57)) ** (3 / 2))
                + -1.3779 * ((1 - (temp / 309.57)) ** (5 / 2))
                + -4.051 * (1 - (temp / 309.57)) ** 5
            )
        )
    )

    ldynvis = 0.0293423 * np.e ** (
        (1.609 * (((309.57 - 5.24) / (temp - 5.24) - 1) ** (1 / 3)))
