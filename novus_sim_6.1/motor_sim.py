...
lden, vden, hl, hg, cp, vap_pres, ldynvis = motor.thermophys(
    temp
...
fuel_mass = (
    (((DIA_FUEL / 2) ** 2) * np.pi - (((DIA_PORT / 2) ** 2) * np.pi))
    * LENGTH_PORT
    * DENSITY_FUEL
)
tmass = lmass + vmass
...
while True:
    ...
    if mdotox > 0 and lmass > 0:
        ...
        vis_pdrop = (f * lden * (flow_speed ** 2) * LENGTH_FEED) / (
            4 * DIA_FEED
        )  # loss in pipe
        ...
        manifold_pres = (
            vap_pres - entry_loss - valve_loss - vis_pdrop
        )  # sum pressure drops
    else:
    ...
    if blowdown_type == "liquid":
        ...
        mdotox1 = NUM_INJ1 * motor.dyer_injector(
            pres_cham, DIA_INJ1, lden, inj_pdrop, hl, manifold_pres, vap_pres
        )
        mdotox2 = NUM_INJ2 * motor.dyer_injector(
            pres_cham, DIA_INJ2, lden, inj_pdrop, hl, manifold_pres, vap_pres
        )
        mdotox3 = NUM_INJ3 * motor.dyer_injector(
            pres_cham, DIA_INJ3, lden, inj_pdrop, hl, manifold_pres, vap_pres
        )
        ...
        lmass_post_vap = (VOL_TANK - (tmass / vden)) / (
            (1 / lden) - (1 / vden)
        ...
        if lmass_pre_vap < lmass_post_vap:  # check for liquid depletion
            ...
            print(
                "injector pressure drop at liquid depletion was",
        ...
        if Z2 == "numerical instability":
            ...
            break
        temp = temp_ld * (
            ((Z2 * vmass) / (Z_ld * vmass_ld)) ** (gamma_N2O - 1)
    ...
    if mdotox / (((DIA_PORT / 2) ** 2) * np.pi) > 600:
        print(
            "Failure: oxidizer flux too high:", mdotox / (((DIA_PORT / 2) ** 2) * np.pi)
        )
        break
    ...
    pres_exit = pres_cham * (1 + (gamma - 1) * mach_exit ** 2 / 2) ** (
        -gamma / (gamma - 1)
    )
...
print(
    "\nPerformance results:\nInitial thrust:",
    thrust_data[int(0.5 / STEP)],
    "N\nmean thrust:",
    np.mean(thrust_data),
    "N\nimpulse:",
    impulse,
    "Ns\nmean Isp:",
    impulse / (prop_mass_data[0] - fuel_mass) / 9.81,
)
...
with open("motor_out.csv", "w", newline="") as motor_file:
    ...
    for i in range(len(time_data)):
        motor_write.writerow(
            [
                ...
                throat_data[i],
                gamma_data[i],
                nozzle_efficiency_data[i],
                exit_pressure_data[i],
                area_ratio_data[i],
                vden_data[i],
                vmass_data[i],
                lden_data[i],
    ...
    motor_write.writerow(
        [
            ...
            throat_data[-1],
            gamma_data[-1],
            nozzle_efficiency_data[-1],
            exit_pressure_data[-1],
            area_ratio_data[-1],
            vden_data[-1],
            0,
            lden_data[-1],
