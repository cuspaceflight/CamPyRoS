...
class Rocket:
    ...
    def body_to_inertial(self, vector, quat_b2i):
        ...
        """      
        q1 = quatmul(np.array([0, vector[0], vector[1], vector[2]]), quat_b2i)
        q2 = quatinv(quat_b2i)
        output = quatmul(q1, q2)[1:]
        return output

    def inertial_to_body(self, vector, quat_b2i):
        ...
        """    
        q1 = quatinv(quat_b2i)
        q2 = quatmul(np.array([0, vector[0], vector[1], vector[2]]), quat_b2i)
        output = quatmul(q1, q2)[1:]
        return output

    def aero_forces(self, alt, quat_b2i, position, velocity, time):
        ...
        v_rel_wind = self.inertial_to_body(velocity-wind_inertial, quat_b2i)
    ...
    def accelerations(self, pos_i, vel_i, w_b, quat_b2i, time):
        ...
        aero_force_b, cop = self.aero_forces(self.altitude(pos_i), quat_b2i, pos_i, vel_i, time)
        ...
        thrust = self.body_to_inertial(thrust_b, quat_b2i)
        aero_force = self.body_to_inertial(aero_force_b, quat_b2i)
    ...
    def w_b_to_quat_b2idot(self, w_b, quat_i2b):
        ''' Converts angular velocity in the body frame to yaw-pitch-roll rates'''
        return 0.5 * quatmul(quat_i2b, np.array([0, w_b[0], w_b[1], w_b[2]]))
    ...
    def step_euler(self):
        ...
        vel_i = self.vel_i
        w_b = self.w_b
        quat_b2i = self.quat_b2i
        time = self.time
        
        self.vel_i = vel_i + self.accelerations(pos_i, vel_i, w_b, quat_b2i, time)[0] * self.h
        self.pos_i = pos_i + self.vel_i * self.h
        
        self.w_b = w_b + self.accelerations(pos_i, vel_i, w_b, quat_b2i, time)[1] * self.h
        
        quat_b2idot = self.w_b_to_quat_b2idot(self.w_b, quat_b2i)
        self.quat_b2i = quat_b2i + quat_b2idot * self.h
    ...
    def check_phase(self):
        if self.on_rail==True:
            ...
            if flight_distance>=self.launch_site.rail_length:
                ...
                np.linalg.norm(self.accelerations(self.pos_i, self.vel_i, self.w_b, self.quat_b2i, self.time)[0])/9.81)
                )
                self.on_rail=False
...
def quat2ypr(q):
    ...
    #Copied from https://stackoverflow.com/questions/54214698/quaternion-to-yaw-pitch-roll
    qw = q[0]
    qx = q[1]
    qy = q[2]
    qz = q[3]
