...
class CylindricalApproximation:
    ...
    def __init__(self, mass_array, time_array, r, l):
        ...
        self.time_array = (
            time_array  # Times since ignition corresponding to the mass datapoints (s)
        )
...
class LiquidTank:
    ...
    def __init__(
        ...
        self.time_array = (
            time_array  # Times since ignition for mass and density datapoints (s)
        )

        self.r = r  # Fuel tank radius (m)
        self.pos_bottom = (
            pos_bottom  # Distance between nose tip and the bottom of the fuel tank
        )
        ...
        if vmass_array != None and vden_array == None:
            raise ValueError(
                "You must input data for both vmass_array and vden_array. Without vden_array data, the position of the vapour COG is unclear"
            )
...
class HollowCylinder:
    ...
    def __init__(self, mass, r_out, r_in, l, cog):
        ...
        self.iyy = self.mass * (
            (self.a ** 2) / 2 + (self.t ** 2) / 8 + (self.l ** 2) / 12
        )
        self.izz = self.iyy
...
class MassModel:
    ...
    def iyy(self, time):
        ...
        for mass_model in self.constants:
            iyy = (
                iyy
                + mass_model.iyy
                + mass_model.mass * (mass_model.cog - self.cog(time)) ** 2
            )
        ...
        for mass_model in self.variables:
            iyy = (
                iyy
                + mass_model.iyy(time)
                + mass_model.mass(time) * (mass_model.cog(time) - self.cog(time)) ** 2
            )
    ...
    def add_solidfuel(self, mass_array, time_array, den, r_out, l, pos_bottom):
        self.variables.append(
            SolidFuel(mass_array, time_array, den, r_out, l, pos_bottom)
        )
