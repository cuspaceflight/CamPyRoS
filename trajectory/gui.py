...
LAUNCH_SITE_INPUTS = {"Rail length (m)" : [float, 5],
...
class Main:
    ...
    def generate_code(self):
        ...
        for i in range(len(object_names)):
            ...
            code_to_add = object_to_code(object_names[i].replace(" ", "_"), object_inputs)
...
class AddObjectWindow(tk.Toplevel):
    ...
    def show_inputs(self, event):
        ...
        for i in range(len(self.input_keys)):
            ...
            if datatype == float or datatype == int or datatype == str or datatype == list:
        ...
        for i in range(len(self.input_entries)):
            ...
            if datatype == float or datatype == int or datatype == str:
                ...
                try:
                    ...
                    popup_message(f"Failed to convert {self.input_entries[i].get()} to a list")
                    raise
                ...
                if type(literal_eval) is list:
                    ...
                    popup_message(f"Failed to convert {self.input_entries[i].get()} to a list")
                    raise TypeError(f"Failed to convert {self.input_entries[i].get()} to a list")
            else:
                dictionary_to_save[self.input_keys[i]] = str(self.input_entries[i].get())
...
def object_to_code(name, dictionary):
    if dictionary["__OBJECT__"] == "Launch Site":
        return (f'{name} = trajectory.LaunchSite('
        ...
        popup_message("Converting the object type '{}' to code has not yet been implemented. This object has been skipped".format(dictionary["__OBJECT__"]))
